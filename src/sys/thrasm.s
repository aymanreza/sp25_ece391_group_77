...
## Inputs: s0 - function entry point
##s1- first operand of the uint32_t a
## s2-second operand of the uint32_t b
## s3-s7-addtional argument
## s8-return the adress for thread_exit
## s9-entry function pointer
#Outputs: a0-this will result of the sum
## Description/Side Effects: It will initalize a thread by movig the saved register to the argyment regsiter.
## setting the return adress, and will jmp to the thread entry function.
##The side effect will be argument registers and the return address.


_thread_startup:
        # FIXME your code goes here
        ## move all the s register to the a register from s0-s7 to a0-a7
        mv a0, s0
        mv a1, s1
        mv a2, s2
        mv a3, s3
        ...
        mv a6, s6
        mv a7, s7
        mv ra, s8 ##will set the return address of the thread_exit function
        jalr x0,s9,0 ## this will jump to the entry function
